...
class TraceProcessor {
        ...
        for (const index of bxEventIndices) {
            ...
            if (event.ph === 'X') {
                ...
                for (let j = startIndex; j < timestampSortedIndices.length; j++) {
                    ...
                    if (!eventMatches)
                        continue;
                    if (potentialMatchingEvent.ph === 'E' &&
    ...
    static getRiskToResponsiveness(events, startTime, endTime, percentiles = [0.5, 0.75, 0.9, 0.99, 1]) {
        ...
        return this._riskPercentiles(ret.durations, totalTime, percentiles, ret.clippedLength);
    }
    /**
    ...
    static getMainThreadTopLevelEvents(trace, startTime = 0, endTime = Infinity) {
        ...
        for (const event of trace.mainThreadEvents) {
            if (!this.isScheduleableTask(event) || !event.dur)
                continue;
    ...
    static findMainFrameIds(events) {
        ...
        if (startedInBrowserEvt &&
            ...
            const threadNameEvt = events.find(e => e.pid === pid &&
                e.ph === 'M' &&
                e.cat === '__metadata' &&
                e.name === 'thread_name' &&
    ...
    static isLCPCandidateEvent(evt) {
        return Boolean(evt.name === 'largestContentfulPaint::Candidate' &&
            evt.args &&
            evt.args.frame &&
            evt.args.data &&
    ...
    static computeValidLCPAllFrames(events, timeOriginEvent) {
        ...
        for (const lcp of finalLcpEventsByFrame.values()) {
            if (!this.isLCPCandidateEvent(lcp))
                continue;
    ...
    static processTrace(trace, options) {
        ...
        /** @return {evt is FrameCommittedEvent} */ evt => {
            return Boolean(evt.name === 'FrameCommittedInBrowser' &&
                evt.args.data &&
                evt.args.data.frame &&
                evt.args.data.url);
        })
        ...
        if (frameIdToRootFrameId.has(mainFrameIds.frameId)) {
            ...
            (0, logger_1.log)('frameTreeEvents may be incomplete, make sure the trace has FrameCommittedInBrowser events');
        ...
        const timeOriginEvt = this.computeTimeOrigin({ keyEvents, frameEvents, mainFrameIds }, timeOriginDeterminationMethod);
        // Subset all trace events to just our tab's process (incl threads other than main)
        // stable-sort events to keep them correctly nested.
        const processEvents = TraceProcessor.filteredTraceSort(trace.traceEvents, e => e.pid === mainFrameIds.pid);
        const mainThreadEvents = processEvents.filter(e => e.tid === mainFrameIds.tid);
    ...
    static processNavigation(processedTrace) {
        ...
        const fcpAllFramesEvt = frameTreeEvents.find(e => e.name === 'firstContentfulPaint' && e.ts > timeOriginEvt.ts);
        if (!fcpAllFramesEvt) {
    ...
    static computeTimeOrigin(traceEventSubsets, method) {
        ...
        const lighthouseMarker = () => {
            ...
            return frameEvents.find(evt => evt.name === 'clock_sync' &&
        ...
        switch (method) {
            case 'firstResourceSendRequest': {
                ...
                if (!fetchStart)
                    throw this.createNoResourceSendRequestError();
                return fetchStart;
            }
            ...
            case 'lighthouseMarker': {
                ...
                if (!marker)
                    throw this.createNoLighthouseMarkerError();
                return marker;
            }
            case 'auto': {
    ...
    static computeNavigationTimingsForFrame(frameEvents, options) {
        ...
        const firstContentfulPaint = frameEvents.find(e => e.name === 'firstContentfulPaint' && e.ts > timeOriginEvt.ts);
        if (!firstContentfulPaint) {
            throw this.createNoFirstContentfulPaintError();
        }
        // fMP will follow at/after the FP
        let firstMeaningfulPaint = frameEvents.find(e => e.name === 'firstMeaningfulPaint' && e.ts > timeOriginEvt.ts);
        ...
        if (!firstMeaningfulPaint) {
            ...
            fmpFellBack = true;
            (0, logger_1.log)(`No firstMeaningfulPaint found, falling back to last ${fmpCand}`);
            const lastCandidate = frameEvents.filter(e => e.name === fmpCand).pop();
            if (!lastCandidate) {
                (0, logger_1.log)('No `firstMeaningfulPaintCandidate` events found in trace');
            }
        ...
        const domContentLoaded = frameEvents.find(e => e.name === 'domContentLoadedEventEnd' && e.ts > timeOriginEvt.ts);
...
exports.default = TraceProcessor;
//# sourceMappingURL=lh-trace-processor.js.map